...
"""根据输入的图像间距（spacing）、图像块大小（patch_size）、最小特征图大小（min_feature_map_size）和最大池化次数（max_numpool），
计算出合适的池化操作核大小（pool_op_kernel_sizes）、卷积核大小（conv_kernel_sizes），同时对图像块大小进行填充，使其满足特定的可除性要求。"""
def get_pool_and_conv_props(spacing, patch_size, min_feature_map_size, max_numpool):
    ...
    return num_pool_per_axis, pool_op_kernel_sizes, conv_kernel_sizes, patch_size, must_be_divisible_by # 每个轴上的池化次数、池化操作的核大小、卷积操作的核大小、调整后的输入块大小、输入块大小必须能被整除的因子
