...
def split_4d(input_folder, num_processes=default_num_threads, overwrite_task_output_id=None):
    assert isdir(join(input_folder, "images")) and isdir(join(input_folder, "labels")) and \
        ...
        "images and labels subfolders and the dataset.json file"
    ...
    shutil.copytree(join(input_folder, "labels"), join(output_folder, "labels"))
...
def create_lists_from_splitted_dataset(base_folder_splitted):
    ...
    json_file = join(base_folder_splitted, "dataset.json")# '/media/lyq/4dbd4ed9-dd80-4bb0-8276-9178451541d2/A2FSeg-main/data/nnUNet_raw_data/Task083_debugBraTS2020/dataset.json'
    with open(json_file) as jsn:
        d = json.load(jsn) # 加载json文件
        training_files = d['training'] # 训练文件
        test_files = d['test'] # 测试文件
    ...
    """
    num_modalities = len(d['modality'].keys()) # 0 1 2 3

    for tr in training_files: # {'image': './images/BraTS20_Training_176.nii', 'label': './labels/BraTS20_Training_176.nii'}
        cur_pat = []
        for mod in range(num_modalities): # ['..../nnUNet_raw_data/Task083_debugBraTS2020/images/BraTS20_Training_176_0000.nii', '..../nnUNet_raw_data/Task083_debugBraTS2020/images/BraTS20_Training_176_0001.nii']
            # cur_pat.append(join(base_folder_splitted, "images", tr['image'].split("/")[-1][:-4] +"_%04.0d.nii" % mod))
            cur_pat.append(join(base_folder_splitted, "images", tr['image'].split("/")[-1][:-7] +"_%04.0d.nii.gz" % mod))
        cur_pat.append(join(base_folder_splitted, "labels", tr['label'].split("/")[-1]))
    ...
    for ts in test_files:
        ...
        for mod in range(num_modalities):
            # cur_pat.append(join(base_folder_splitted, "imagesTs", ts['image'].split("/")[-1][:-4] +"_%04.0d.nii" % mod))
            cur_pat.append(join(base_folder_splitted, "imagesTs", ts['image'].split("/")[-1][:-7] +"_%04.0d.nii.gz" % mod))
        cur_pat.append(join(base_folder_splitted, "labelsTs", ts['label'].split("/")[-1]))
        lists.append(cur_pat)
    
    return lists, {int(i): d['modality'][str(i)] for i in d['modality'].keys()} # 训练+验证+测试集文件与其标签 [[序列1，序列2，序列3，序列4，标签],.....] ， {0: 'T1', 1: 'T1ce', 2: 'T2', 3: 'FLAIR'}
...
def crop(task_string, override=False, num_threads=default_num_threads):
    cropped_out_dir = join(nnUNet_cropped_data, task_string) # '..../nnUNet_cropped_data/任务ID_debugBraTS2020'
    maybe_mkdir_p(cropped_out_dir)

    if override and isdir(cropped_out_dir): # False
        shutil.rmtree(cropped_out_dir)
        maybe_mkdir_p(cropped_out_dir)

    splitted_4d_output_dir_task = join(nnUNet_raw_data, task_string) # '..../nnUNet_raw_data/任务ID_debugBraTS2020'
    lists, _ = create_lists_from_splitted_dataset(splitted_4d_output_dir_task) # 训练+验证+测试集文件与其标签 [[序列1，序列2，序列3，序列4，标签]....] ， {0: 'T1', 1: 'T1ce', 2: 'T2', 3: 'FLAIR'}（舍弃）

    imgcrop = ImageCropper(num_threads, cropped_out_dir) # 创建裁剪对象
    imgcrop.run_cropping(lists, overwrite_existing=override) # 裁剪 训练+验证+测试集文件 (包括裁剪图像和标签bbox，一起堆叠保存为npz文件，标签的全0区域 改成-1，为了不计算损失（猜的）)

    shutil.copy(join(nnUNet_raw_data, task_string, "dataset.json"), cropped_out_dir) # 把data.json文件 复制到  '..../nnUNet_cropped_data/任务ID_debugBraTS2020' 下
