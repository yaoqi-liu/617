...
class DatasetAnalyzer(object):
    ...
    """获取data.json文件中的  labels": {"0": "background", "1": "edema", "2": "non-enhancing",  "3": "enhancing"} """
    ...
    """获取properties文件中的裁剪后的size和原spacing"""
    ...
    def get_modalities(self):
        ...
        modalities = {int(k): modalities[k] for k in modalities.keys()} # {0: 'T1', 1: 'T1ce', 2: 'T2', 3: 'FLAIR'}
        return modalities
    """计算每个患者数据在裁剪 前后 的大小缩减比例"""
    def get_size_reduction_by_cropping(self):
        ...
        for p in self.patient_identifiers:
            ...
            shape_before_crop = props["original_size_of_raw_data"] # 原图 size
            shape_after_crop = props['size_after_cropping'] # bbox size
            size_red = np.prod(shape_after_crop) / np.prod(shape_before_crop) #  bbox(D*H*W) / ori（D*H*W）
            size_reduction[p] = size_red
        return size_reduction #返回 每个文件 计算每个图像文件数据在裁剪前后的大小缩减比例
    ...
    def analyze_dataset(self, collect_intensityproperties=True):
        ...
        sizes, spacings = self.get_sizes_and_spacings_after_cropping() # 获取properties文件中的裁剪后的size和原spacing"

        # get all classes and what classes are in what patients
        # class min size
        # region size per class
        classes = self.get_classes() #获取data.json中的labels {'0': 'background', '1': 'edema', '2': 'non-enhancing', '3': 'enhancing'}
        all_classes = [int(i) for i in classes.keys() if int(i) > 0] # 获取肿瘤的标签类别 [1,2,3]

        # modalities
        modalities = self.get_modalities() #获取模态 {0: 'T1', 1: 'T1ce', 2: 'T2', 3: 'FLAIR'}

        # collect intensity information
        if collect_intensityproperties: # False
            intensityproperties = self.collect_intensity_properties(len(modalities))
        else:
            intensityproperties = None

        # size reduction by cropping
        size_reductions = self.get_size_reduction_by_cropping() # 每个图像文件 计算添加每个图像数据在裁剪bbox前后的大小缩减比例

        dataset_properties = dict()
        dataset_properties['all_sizes'] = sizes #图像 bbox 的size
        dataset_properties['all_spacings'] = spacings #图像 原spacing
        dataset_properties['all_classes'] = all_classes # 肿瘤类别 [1,2,3]
        dataset_properties['modalities'] = modalities  # {idx: modality name} 模态 {0: 'T1', 1: 'T1ce', 2: 'T2', 3: 'FLAIR'}
        dataset_properties['intensityproperties'] = intensityproperties # None
        dataset_properties['size_reductions'] = size_reductions  # {patient_id: size_reduction} {文件名前缀：图像数据在裁剪bbox前后的大小缩减比例}

        save_pickle(dataset_properties, join(self.folder_with_cropped_data, "dataset_properties.pkl")) # 保存为一个pkl文件
