...
def create_lists_from_splitted_dataset(base_folder_splitted):
    ...
    for tr in training_files:
        ...
        for mod in range(num_modalities):
            if "DCE" in tr['image']:
                ...
                # 如果既不包含 DCE 也不包含 T2，按照原逻辑使用 %04.0d 格式化
                suffix = "%04.0d" % mod

            cur_pat.append(os.path.join(base_folder_splitted, "images", tr['image'].split("/")[-1][:-7] +
                                        f"_{suffix}.nii.gz"))
        cur_pat.append(os.path.join(base_folder_splitted, "labels", tr['label'].split("/")[-1]))
    ...
    for val in val_files:
        ...
        for mod in range(num_modalities):
            if "DCE" in val['image']:
                suffix = "0000"
            elif "T2" in val['image']:
                suffix = "0001"
            else:
                # 如果既不包含 DCE 也不包含 T2，按照原逻辑使用 %04.0d 格式化
                suffix = "%04.0d" % mod

            cur_pat.append(os.path.join(base_folder_splitted, "imagesVal", val['image'].split("/")[-1][:-7] +
                                        f"_{suffix}.nii.gz"))
    ...
    for ts in test_files:
        ...
        for mod in range(num_modalities):
            ...
            cur_pat.append(os.path.join(base_folder_splitted, "imagesTs", ts['image'].split("/")[-1][:-7] +
                                        f"_{suffix}.nii.gz"))
...
def crop(task_string, override=False, num_threads=default_num_threads):
    cropped_out_dir = join(nnUNet_cropped_data, task_string) # '/media/lyq/4dbd4ed9-dd80-4bb0-8276-9178451541d2/A2FSeg-main/data/nnUNet_cropped_data/  + Task131_DCET2Reg'
    ...
    splitted_4d_output_dir_task = join(nnUNet_raw_data, task_string) # '/media/lyq/4dbd4ed9-dd80-4bb0-8276-9178451541d2/A2FSeg-main/data/nnUNet_raw_data/Task131_DCET2Reg'
