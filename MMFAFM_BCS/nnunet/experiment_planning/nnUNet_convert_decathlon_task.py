...
from nnunet.experiment_planning.my_utils import split_4d
...
def crawl_and_remove_hidden_from_decathlon(folder):
    ...
    assert folder.split('/')[-1].startswith("Task"), "This does not seem to be a decathlon folder. Please give me a " \
                                                     "folder that starts with TaskXX and has the subfolders images, " \
                                                     "labels and imagesTs"
    subf = subfolders(folder, join=False)
    assert 'images' in subf, "This does not seem to be a decathlon folder. Please give me a " \
                                                     "folder that starts with TaskXX and has the subfolders images, " \
                                                     "labels and imagesTs"
    assert 'imagesTs' in subf, "This does not seem to be a decathlon folder. Please give me a " \
                                                     "folder that starts with TaskXX and has the subfolders images, " \
                                                     "labels and imagesTs"
    assert 'labels' in subf, "This does not seem to be a decathlon folder. Please give me a " \
                                                     "folder that starts with TaskXX and has the subfolders images, " \
                                                     "labels and imagesTs"
    _ = [os.remove(i) for i in subfiles(folder, prefix=".")]
    _ = [os.remove(i) for i in subfiles(join(folder, 'images'), prefix=".")]
    _ = [os.remove(i) for i in subfiles(join(folder, 'labels'), prefix=".")]
