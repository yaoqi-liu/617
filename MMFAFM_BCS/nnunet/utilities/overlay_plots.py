...
def generate_overlays_for_task(task_name_or_id, output_folder, num_processes=8, modality_idx=0, use_preprocessed=True,
    ...
    if not use_preprocessed:
        ...
        identifiers = [i[:-7] for i in subfiles(join(folder, 'labels'), suffix='.nii.gz', join=False)]

        image_files = [join(folder, 'images', i + "_%04.0d.nii.gz" % modality_idx) for i in identifiers]
        seg_files = [join(folder, 'labels', i + ".nii.gz") for i in identifiers]
