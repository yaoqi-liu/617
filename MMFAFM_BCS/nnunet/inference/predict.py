...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    assert isinstance(trainer, nnUNetTrainer_breast)
    q = Queue(2)
    processes = []
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    trainer, params = load_model_and_checkpoint_files(model, folds,mixed_precision=True, checkpoint_name=checkpoint_name)

    print("starting preprocessing generator")
    preprocessing = preprocess_multithreaded(trainer, list_of_lists, cleaned_output_files, num_threads_preprocessing,
                                             segs_from_prev_stage)

    trainer.network = medcam.inject(trainer.network, label=0, replace=True, backend="gcam", layer='lastconv.0')
    ...
    for preprocessed in preprocessing:
        ...
        results.append(pool.starmap_async(save_segmentation_nifti_from_sigmod,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', "--output_folder",
                        default='', help="folder for saving predictions")
    ...
    parser.add_argument('-f', '--folds', nargs='+', default=[], help="folds to use for prediction. Default is None "
