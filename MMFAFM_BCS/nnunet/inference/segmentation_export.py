...
"""将模型预测得到的软最大值（softmax）结果转换为 nifti 格式的分割图像并保存，同时还可以将其保存为 npz 格式"""
def save_segmentation_nifti_from_sigmod(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    :return:
    """
    if verbose: print("force_separate_z:", force_separate_z, "interpolation order:", order) # True None 0

    if isinstance(segmentation_softmax, str): # False
    ...
    current_shape = segmentation_softmax.shape # (3, 140, 187, 134)
    shape_original_after_cropping = properties_dict.get('size_after_cropping') # 从图像pkl文件 获取图像bbox的尺寸 (140, 187, 134)
    shape_original_before_cropping = properties_dict.get('original_size_of_raw_data') # 从图像pkl文件 获取图像原尺寸 (155 240 240)
    # current_spacing = dct.get('spacing_after_resampling')
    # original_spacing = dct.get('original_spacing')
    # 如果 当前 预测结果的尺寸  与 bbox图像尺寸不匹配 （一般都是匹配的）
    if np.any([i != j for i, j in zip(np.array(current_shape[1:]), np.array(shape_original_after_cropping))]):
        ...
        if verbose: print("no resampling necessary") # 没有重采样的必要
        seg_old_spacing = segmentation_softmax # 直接使用预测结果的尺寸 (3, 140, 187, 134)

    if resampled_npz_fname is not None: # False
    ...
    # if region_class_order is None: # False
    #     seg_old_spacing = seg_old_spacing.argmax(0)
    # else:
    #     seg_old_spacing_final = np.zeros(seg_old_spacing.shape[1:]) # (3, 140, 187, 134) -> (140, 187, 134)
    #     for i, c in enumerate(region_class_order): # (1, 2, 3) 遍历 分割类别
    #         seg_old_spacing_final[seg_old_spacing[i] > 0.5] = c # 遍历(3, 140, 187, 134) 的通道 赋值到  (140, 187, 134)
    #     seg_old_spacing = seg_old_spacing_final # 赋值标签值后的 (140, 187, 134) 结果
    seg_old_spacing = seg_old_spacing[0] * 255

    bbox = properties_dict.get('crop_bbox') # 获取预处理裁剪的bbox坐标
    # 预测出来的bbox 按坐标位置放到 全0张量 中
    if bbox is not None: # True
        seg_old_size = np.zeros(shape_original_before_cropping, dtype=np.uint8) # (155 240 240) 创建与原图一样尺寸的 全0张量
        for c in range(3):
    ...
    if seg_postprogess_fn is not None: # False
        seg_old_size_postprocessed = seg_postprogess_fn(np.copy(seg_old_size), *seg_postprocess_args)
    else:
        seg_old_size_postprocessed = seg_old_size # (155,240,240)
    # 把预测结果 转np 并且 设置 原信息
    seg_resized_itk = sitk.GetImageFromArray(seg_old_size_postprocessed.astype(np.uint8))
    seg_resized_itk.SetSpacing(properties_dict['itk_spacing'])
    seg_resized_itk.SetOrigin(properties_dict['itk_origin'])
    seg_resized_itk.SetDirection(properties_dict['itk_direction'])
    sitk.WriteImage(seg_resized_itk, out_fname) # '..../validation_raw/1000/BraTS20_Training_032.nii'

    if (non_postprocessed_fname is not None) and (seg_postprogess_fn is not None): # False
