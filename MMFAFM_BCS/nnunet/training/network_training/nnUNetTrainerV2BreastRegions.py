...
from nnunet.training.network_training.nnUNetTrainerV2_breast import nnUNetTrainerV2_breast
from nnunet.utilities.distributed import awesome_allgather_function
from nnunet.utilities.to_torch import maybe_to_torch, to_cuda


class nnUNetTrainerV2BreastRegions_BN(nnUNetTrainerV2_breast):
...
class nnUNetTrainerV2BreastRegions(nnUNetTrainerV2_breast):
            ...
            if training:
                ...
                if self.unpack_data:
                    ...
                    print(
                        ...
                        "will wait all winter for your model to finish!")
                print(self.data_aug_params)
                _, _, self.ts_gen = get_moreDA_augmentation(self.dl_tr, self.dl_val, self.dl_ts,
                                                                                 self.data_aug_params[
                                                                                     'patch_size_for_spatialtransform'],
                                                                                 self.data_aug_params,
                                                                                 deep_supervision_scales=self.deep_supervision_scales,
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True,
        ...
        # run breast specific validation
        # 未后处理的结果
...
class nnUNetTrainerV2BreastRegions_Dice(nnUNetTrainerV2BreastRegions):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False):
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)
...
class nnUNetTrainerV2BreastRegions_DDP(nnUNetTrainerV2_DDP):
