...
from nnunet.inference.segmentation_export import save_segmentation_nifti_from_sigmod
...
from nnunet.training.dataloading.dataset_loading_breast import load_dataset, DataLoader3D, DataLoader2D, unpack_dataset, load_dataset_each,load_dataset_each_external_val
...
class nnUNetTrainer_breast(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        """
        super(nnUNetTrainer_breast, self).__init__(deterministic, fp16)
    ...
    def update_fold(self, fold):
        ...
        if fold is not None: # True
            if isinstance(fold, str):
                ...
                if self.output_folder.endswith("fold_%s" % str(self.fold)): # False
                    self.output_folder = self.output_folder_base
                self.output_folder = join(self.output_folder, "fold_%s" % str(fold)) # 每一折的输出文件夹 '/media/lyq/4dbd4ed9-dd80-4bb0-8276-9178451541d2/A2FSeg-main/data/RESULTS_FOLDER/nnUNet/3d_fullres/Task131_DCET2Reg/MAML3_channelTrainerV2BreastRegions__nnUNetPlansv2.1/fold_1'
                # print(f"训练模型保存的位置为：{self.output_folder}")
            self.fold = fold
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if training:
            # '.../nnUNet_preprocessed/Task082_BraTS2020/nnUNetData_plans_v2.1_stage0'
    ...
    def save_debug_information(self):
        ...
        for k in self.__dir__():
            if not k.startswith("__"): # 检查属性或方法的名称是否不以 __ 开头，这是为了排除 Python 内置的特殊属性和方法（如 __init__、__len__ 等）
                if not callable(getattr(self, k)): # 通过 callable 函数检查该值是否可调用。如果不可调用，说明它是一个属性（而不是方法），将其属性名 k 和属性值（转换为字符串类型）存储在 dct 字典中。
                    dct[k] = str(getattr(self, k))
        del dct['plans'] # 删除一些特定的属性，这些属性可能包含大量数据或敏感信息，不适合在调试信息中保存
        del dct['intensity_properties']
        del dct['dataset']
        del dct['dataset_tr']
        del dct['dataset_val']
        del dct['dataset_ts']
        save_json(dct, join(self.output_folder, "debug.json")) #保存为debug.json文件 '....RESULTS_FOLDER/nnUNet/3d_fullres/Task083_debugBraTS2020/MAML3_channelTrainerV2BraTSRegions__nnUNetPlansv2.1_batch_01'

        import shutil
        # '..../nnUNet_preprocessed/Task083_debugBraTS2020/nnUNetPlansv2.1_batch_01_plans_3D.pkl' 复制到 ..../RESULTS_FOLDER/nnUNet/3d_fullres/Task083_debugBraTS2020/MAML3_channelTrainerV2BraTSRegions__nnUNetPlansv2.1_batch_01 下
        shutil.copy(self.plans_file, join(self.output_folder_base, "plans.pkl"))
    """breast父类 开始训练"""
    def run_training(self):
        self.save_debug_information()
        super(nnUNetTrainer_breast, self).run_training()
    """加载nnUNet_preprocessed 下的 图像pkl文件"""
    ...
    def process_plans(self, plans):
        ...
        self.patch_size = np.array(stage_plans['patch_size']).astype(int) # patch_size
        ...
        self.base_num_features = 16 # my的 改动 涨点
    ...
    def load_dataset(self):
        # 读取 splits_final.json 文件
        # 数据集文件夹路径(训练+验证)
        base_folder = self.dataset_directory.replace('nnUNet_preprocessed','nnUNet_raw_data')
        self.print_to_log_file("加载数据集的目录为:\n %s" % (base_folder),
                               also_print_to_console=True)
        with open(f'{base_folder}/splits_final.json', 'r') as f:
            splits = json.load(f)

        # 根据 self.fold 参数选择对应的划分
        selected_split = splits[self.fold]
        train_ids = selected_split['train']
        val_ids = selected_split['val']

        # 构建训练集和验证集文件路径列表
        def get_file_paths(ids, folder):
            return [os.path.join(folder, f"{id_}.nii.gz") for id_ in ids]

        train_val_folder = os.path.join(base_folder, 'labels')
        ts_folder = os.path.join(base_folder, 'labelsTs')

        dataset_tr = get_file_paths(train_ids, train_val_folder)
        dataset_val = get_file_paths(val_ids, train_val_folder)
        dataset_ts =  [os.path.join(ts_folder,file) for file in os.listdir(ts_folder)]# 如果测试集不在 splits_final.json 中，可以根据实际情况修改

        self.dataset_tr, self.dataset_val, self.dataset_ts = load_dataset_each(dataset_tr, dataset_val, dataset_ts,
    ...
    def preprocess_predict_nifti(self, input_files: List[str], output_file: str = None,
                                 sigmod_ouput_file: str = None, mixed_precision: bool = True) -> None:

        print("preprocessing...")
        d, s, properties = self.preprocess_patient(input_files)
        print("predicting...")
        pred = self.predict_preprocessed_data_return_seg_and_sigmod(d, do_mirroring=self.data_aug_params["do_mirror"],
        ...
        save_segmentation_nifti_from_sigmod(pred, output_file, properties, interpolation_order,
                                             self.regions_class_order, None, None, sigmod_ouput_file,
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
                 save_softmax: bool = True, use_gaussian: bool = True, overwrite: bool = True, # 强制重新处理所有数据
        ...
        # be used later
        gt_nifti_folder = join(self.output_folder, "gt_niftis")
    ...
    def run_online_evaluation(self, output, target):
        with torch.no_grad():
            ...
            output_softmax = sigmid_helper(output)
    ...
    """训练完统计run_online_evaluation计算出来的指标 返回并写入文件"""
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        super(nnUNetTrainer_breast, self).save_checkpoint(fname, save_optimizer)
        info = OrderedDict()
