...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.num_batches_per_epoch = 250 # 代表每个训练轮次中的批次数
        self.num_val_batches_per_epoch = 50 # 表示每个验证轮次中的批次数
    ...
    """绘制训练和验证过程中的损失曲线以及验证评估指标曲线，并将绘制好的图保存为 progress.png 文件"""
    ...
    def print_to_log_file(self, *args, also_print_to_console=True, add_timestamp=True):
        ...
        while not successful and ctr < max_attempts:
            try:
                with open(self.log_file, 'a+') as f: # training_log.txt
                    for a in args:
                        f.write(str(a))
                        f.write(" ")
                    f.write("\n")
    ...
    def load_checkpoint(self, fname, train=True):
        ...
        if not self.was_initialized: # False
            self.initialize(train)
        # saved_model = torch.load(fname, map_location=torch.device('cuda', torch.cuda.current_device()))
        saved_model = torch.load(fname, map_location=torch.device('cpu'),weights_only=False) # 改动 best epoch 799???
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        """
        pass
    """用于从已经加载到内存中的检查点（checkpoint）恢复模型的状态，包括模型参数、优化器状态、学习率调度器状态等，
        同时处理可能存在的状态字典键不匹配问题以及旧模型训练轮次计数的错误。"""
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        for k, value in checkpoint['state_dict'].items():
            key = k
            if key not in curr_state_dict_keys and key.startswith('module.'): # False
                key = key[7:]
        ...
        if self.epoch != len(self.all_tr_losses): # False
        ...
        self._maybe_init_amp()
    """实例初始化自动混合精度训练所需的梯度缩放器，以便后续在训练过程中能够正确使用自动混合精度技术"""
    def _maybe_init_amp(self):
        if self.fp16 and self.amp_grad_scaler is None: # True
    ...
    def run_training(self):
        if not torch.cuda.is_available(): # False
            self.print_to_log_file("WARNING!!! You are attempting to run training on a CPU (torch.cuda.is_available() is False). This can be very slow!")

        _ = self.tr_gen.next() #训练图像 data (1,4,128,128,128) properties信息 keys ['BraTS20_Training_172'] target (1,3,128,128,128) (1,3,64,64,64) (1,3,32,32,32) (1,3,16,16,16) (1,3,8,8,8)
        _ = self.val_gen.next() # 验证图像
        ...
        maybe_mkdir_p(self.output_folder) # '.../RESULTS_FOLDER/nnUNet/3d_fullres/Task082_BraTS2020/MAML3_channelTrainerV2BraTSRegions__nnUNetPlansv2.1_batch_01'
        self.plot_network_architecture()
        ...
        if not self.was_initialized: # False
            self.initialize(True)
        print(f"max_epoch:{self.max_num_epochs}")
        while self.epoch < self.max_num_epochs:
            self.print_to_log_file("\nepoch: ", self.epoch)
            epoch_start_time = time()
            train_losses_epoch = []
            ...
            with torch.no_grad():
                ...
                if self.also_val_in_tr_mode:
                    ...
                    # validation with train=True
                    val_losses = []
                    for _ in tqdm(range(self.num_val_batches_per_epoch)):
                        l = self.run_iteration(self.val_gen, False)
                        val_losses.append(l)
                    self.all_val_losses_tr_mode.append(np.mean(val_losses))
                    self.print_to_log_file("validation loss (train=True): %.4f" % self.all_val_losses_tr_mode[-1])
        ...
        if self.save_final_checkpoint:  # True
    ...
    def update_eval_criterion_MA(self):
        ...
        :return:
        """
        if self.val_eval_criterion_MA is None: # False
    ...
    def on_epoch_end(self):
        ...
        continue_training = self.manage_patience() # True
        return continue_training
    def update_train_loss_MA(self):
        if self.train_loss_MA is None: # False
