...
def convert_to_npy(args):
    if not isinstance(args, tuple): # False
        key = "data"
        npz_file = args
    else:
        npz_file, key = args # '.../nnUNet_preprocessed/Task083_debugBraTS2020/nnUNetData_plans_v2.1_stage0/BraTS20_Training_085.npz' , data
    if not isfile(npz_file[:-3] + "npy"): # 如果npy文件不存在（生成过就不重复生成了）则生成
...
def unpack_dataset(folder, threads=default_num_threads, key="data"):
    ...
    """
    p = Pool(threads)
    npz_files = subfiles(folder, True, None, ".npz", True) # ['.../nnUNet_preprocessed/Task083_debugBraTS2020/nnUNetData_plans_v2.1_stage0/BraTS20_Training_001.npz',......]
    p.map(convert_to_npy, zip(npz_files, [key] * len(npz_files))) # convert_to_npy参数 .../nnUNet_preprocessed/Task083_debugBraTS2020/nnUNetData_plans_v2.1_stage0/BraTS20_Training_001.npz , data
    p.close()
    p.join()
...
def load_dataset_each_external_val(dataset_val,  folder, num_cases_properties_loading_threshold=1000):
    # we don't load the actual data but instead return the filename to the np file.
    print('loading dataset')
    val_case_identifiers = get_case_identifiers_from_raw_folder_label(dataset_val)
...
#----------------val------------------
    ...
    if len(val_case_identifiers) <= num_cases_properties_loading_threshold:
        print('loading val all case properties')
...
def get_case_identifiers_from_raw_folder_label(folder):
    # case_identifiers = np.unique([i[:-7] for i in os.listdir(folder) if i.endswith(".nii.gz") and (i.find("segFromPrevStage") == -1)])
    case_identifiers = np.unique([i.split('/')[-1][:-7]  for i in folder if i.endswith(".nii.gz") and (i.find("segFromPrevStage") == -1)])
    return case_identifiers

def load_dataset_each(dataset_tr, dataset_val, dataset_ts, folder, num_cases_properties_loading_threshold=1000):
    # we don't load the actual data but instead return the filename to the np file.
    print('loading dataset')
    tr_case_identifiers = get_case_identifiers_from_raw_folder_label(dataset_tr)
    val_case_identifiers = get_case_identifiers_from_raw_folder_label(dataset_val)
    ts_case_identifiers = get_case_identifiers_from_raw_folder_label(dataset_ts)
...
#----------------train------------------
    for c in tr_case_identifiers:
        ...
        dataset_tr[c]['data_file'] = join(folder, "%s.npz" % c) # OrderedDict([('BraTS20_Training_003', OrderedDict([('data_file', '/media/lyq/4dbd4ed9-dd80-4bb0-8276-9178451541d2/A2FSeg-main/data/nnUNet_preprocessed/Task082_BraTS2020/nnUNetData_plans_v2.1_stage0/BraTS20_Training_003.npz')]))])

        # dataset[c]['properties'] = load_pickle(join(folder, "%s.pkl" % c))
        dataset_tr[c]['properties_file'] = join(folder, "%s.pkl" % c) # '/media/lyq/4dbd4ed9-dd80-4bb0-8276-9178451541d2/A2FSeg-main/data/nnUNet_preprocessed/Task082_BraTS2020/nnUNetData_plans_v2.1_stage0/BraTS20_Training_003.pkl'
    ...
    if len(tr_case_identifiers) <= num_cases_properties_loading_threshold:
        print('loading tr all case properties')
...
#----------------test------------------
    ...
    if len(ts_case_identifiers) <= num_cases_properties_loading_threshold:
        print('loading ts all case properties')
...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def determine_shapes(self):
        if self.has_prev_stage: # False
            num_seg = 2
        else:
            num_seg = 1

        k = list(self._data.keys())[0] # SY2_10403063 在nnUNet_preprocessed的预处理npz中随机选择一个文件名前缀
        if isfile(self._data[k]['data_file'][:-4] + ".npy"):
            case_all_data = np.load(self._data[k]['data_file'][:-4] + ".npy", self.memmap_mode) # (3,150,319,319) 读取SY2_10403063.npy文件
        else:
            case_all_data = np.load(self._data[k]['data_file'])['data']
        num_color_channels = case_all_data.shape[0] - 1 # 最后一个通道是分割标签，所以减去 1
        data_shape = (self.batch_size, num_color_channels, *self.patch_size) #(1,2,194,289,210)  patch_size(194,289,210)???
        seg_shape = (self.batch_size, num_seg, *self.patch_size)#(1,2,194,289,210)
        return data_shape, seg_shape
    def generate_train_batch(self):
        selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None) # 随机选择一个图像名
        data = np.zeros(self.data_shape, dtype=np.float32) # (1, 2, 194, 289, 210)
        seg = np.zeros(self.seg_shape, dtype=np.float32) # (1, 1, 194, 289, 210)
        case_properties = []
        for j, i in enumerate(selected_keys):
            # oversampling foreground will improve stability of model training, especially if many patches are empty
            # (Lung for example)
            if self.get_do_oversample(j): # False
                force_fg = True
            else:
                force_fg = False

            if 'properties' in self._data[i].keys():
                properties = self._data[i]['properties'] # 访问当前图像的properties信息
            else:
            ...
            if isfile(self._data[i]['data_file'][:-4] + ".npy"):
                case_all_data = np.load(self._data[i]['data_file'][:-4] + ".npy", self.memmap_mode) # (3, 204, 340, 340)
            else:
