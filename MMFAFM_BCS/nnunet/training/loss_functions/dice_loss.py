...
"""计算预测结果（net_output）和真实标签（gt）之间的真阳性（True Positives, TP）、假阳性（False Positives, FP）、假阴性（False Negatives, FN）和真阴性（True Negatives, TN）"""
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    shp_y = gt.shape # (B,C,D,H,W)

    with torch.no_grad():
        if len(shp_x) != len(shp_y): # False
            gt = gt.view((shp_y[0], 1, *shp_y[1:]))

        if all([i == j for i, j in zip(net_output.shape, gt.shape)]): # True
            # if this is the case then gt is probably already a one hot encoding
            y_onehot = gt
        else:
            gt = gt.long()
...
class DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
        ...
        if not square_dice:
            self.dc = SoftDiceLoss(apply_nonlin=sigmid_helper, **soft_dice_kwargs)
        else:
            self.dc = SoftDiceLossSquared(apply_nonlin=sigmid_helper, **soft_dice_kwargs)
...
class GDL_and_CE_loss(nn.Module):
    def __init__(self, gdl_dice_kwargs, ce_kwargs, aggregate="sum"):
        ...
        self.aggregate = aggregate
        self.ce = RobustCrossEntropyLoss(**ce_kwargs)
        self.dc = GDL(sigmid_helper, **gdl_dice_kwargs)
