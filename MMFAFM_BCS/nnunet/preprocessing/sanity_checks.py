...
def verify_dataset_integrity(folder):
    """
    folder needs the images, imagesTs and labels subfolders. There also needs to be a dataset.json
    ...
    assert isdir(join(folder, "images")), "There needs to be a images subfolder in folder, folder=%s" % folder
    assert isdir(join(folder, "labels")), "There needs to be a labels subfolder in folder, folder=%s" % folder
    ...
    ## check training set
    nii_files_in_imagesTr = subfiles((join(folder, "images")), suffix=".nii.gz", join=False)
    nii_files_in_labelsTr = subfiles((join(folder, "labels")), suffix=".nii.gz", join=False)
    ...
    for c in expected_train_identifiers:
        ...
        expected_label_file = join(folder, "labels", c + ".nii.gz")
        label_files.append(expected_label_file)
        expected_image_files = [join(folder, "images", c + "_%04.0d.nii.gz" % i) for i in range(num_modalities)]
    ...
    assert len(
        nii_files_in_imagesTr) == 0, "there are training cases in images that are not listed in dataset.json: %s" % nii_files_in_imagesTr
    assert len(
        nii_files_in_labelsTr) == 0, "there are training cases in labels that are not listed in dataset.json: %s" % nii_files_in_labelsTr
    ...
    all_same, unique_orientations = verify_all_same_orientation(join(folder, "images"))
    if not all_same:
        print(
