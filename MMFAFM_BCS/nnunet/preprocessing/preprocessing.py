...
class GenericPreprocessor(object):
    def __init__(self, normalization_scheme_per_modality, use_nonzero_mask, transpose_forward: (tuple, list), intensityproperties=None):
        ...
        """
        self.transpose_forward = transpose_forward # [0,1,2]
        self.intensityproperties = intensityproperties # False
    ...
    def _run_internal(self, target_spacing, case_identifier, output_folder_stage, cropped_output_dir, force_separate_z,
                      all_classes):
        data, seg, properties = self.load_cropped(cropped_output_dir, case_identifier) # （4，D,W,H） (1，D,W,H) 图像信息

        data = data.transpose((0, *[i + 1 for i in self.transpose_forward]))
        seg = seg.transpose((0, *[i + 1 for i in self.transpose_forward]))
        # 对image和label进行重采样到  新的间距（之前nnunet自己计算的合适的间距）
        data, seg, properties = self.resample_and_normalize(data, target_spacing,
                                                            properties, seg, force_separate_z)

        all_data = np.vstack((data, seg)).astype(np.float32) # 重采样完 image和label 再堆叠到一起（4+1，D,W,H）
        ...
        for c in all_classes:
            all_locs = np.argwhere(all_data[-1] == c) # 使用 np.argwhere 函数找出 all_data 最后一个通道（通常是标签数据）中等于当前类别 c 的所有位置
            if len(all_locs) == 0: # 如果该类别在标签数据中不存在（即 all_locs 为空），则将该类别对应的采样位置设为空列表，并跳过本次循环
                class_locs[c] = []
                continue
            target_num_samples = min(num_samples, len(all_locs)) # 取 num_samples 和该类别总位置数量的较小值，确保采样数量不超过该类别实际存在的位置数量
            target_num_samples = max(target_num_samples, int(np.ceil(len(all_locs) * min_percent_coverage))) #进一步调整目标采样数量，取前面计算结果和该类别总位置数量的 1%（向上取整）的较大值，确保采样数量满足最小覆盖比例要求。

            selected = all_locs[rndst.choice(len(all_locs), target_num_samples, replace=False)]
            class_locs[c] = selected # 采样得到的标签类别位置信息存储
            print(c, target_num_samples) # 当前类别和目标采样数量
        ...
        np.savez_compressed(os.path.join(output_folder_stage, "%s.npz" % case_identifier),
                            data=all_data.astype(np.float32)) # 把 图像和标签的堆叠文件 保存 在nnUNet_preprocessed下的npz文件
        with open(os.path.join(output_folder_stage, "%s.pkl" % case_identifier), 'wb') as f:
            pickle.dump(properties, f) # 把 采样得到的标签类别位置信息 保存在在nnUNet_preprocessed下的pkl文件下
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        print("npz folder:", input_folder_with_cropped_npz) # '..../data/nnUNet_cropped_data/Task083_debugBraTS2020'
        print("output_folder:", output_folder) # '..../data/nnUNet_preprocessed/Task083_debugBraTS2020'
        list_of_cropped_npz_files = subfiles(input_folder_with_cropped_npz, True, None, ".npz", True) # nnUNet_cropped_data文件夹下的所有npz文件
        maybe_mkdir_p(output_folder)
        num_stages = len(target_spacings) # 1
        if not isinstance(num_threads, (list, tuple, np.ndarray)):
            num_threads = [num_threads] * num_stages # [8]
        ...
        all_classes = load_pickle(join(input_folder_with_cropped_npz, 'dataset_properties.pkl'))['all_classes'] # 记载dataset_properties.pkl分割类别数 [1,2,3]

        for i in range(num_stages):
            all_args = []
            output_folder_stage = os.path.join(output_folder, data_identifier + "_stage%d" % i) # .../data/nnUNet_preprocessed/Task083_debugBraTS2020/nnUNetData_plans_v2.1_stage0
            maybe_mkdir_p(output_folder_stage)
            spacing = target_spacings[i] # [1. 1. 1.]
            for j, case in enumerate(list_of_cropped_npz_files):
                case_identifier = get_case_identifier_from_npz(case) # 'BraTS20_Training_001'
