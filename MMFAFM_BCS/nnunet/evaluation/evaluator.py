...
class Evaluator:
    ...
    """接受不同类型的输入（字典、集合、numpy 数组、列表或元组），并根据输入类型进行相应的处理，将标签信息存储在评估器的 self.labels 属性中"""
    def set_labels(self, labels):
        ...
        elif isinstance(labels, (list, tuple)): # that
            self.labels = labels
        else:
...
def run_evaluation(args):
    ...
    # evaluate
    evaluator.set_test(test) # 赋值到 self.test
    evaluator.set_reference(ref) # 赋值到 self.ref
    if evaluator.labels is None: # False
...
def aggregate_scores(test_ref_pairs,
    ...
    if labels is not None:
        evaluator.set_labels(labels) # 把labels 赋值到 self.labels中

    all_scores = OrderedDict()
    all_scores["all"] = []
    all_scores["mean"] = OrderedDict()

    test = [i[0] for i in test_ref_pairs] # 预测
    ref = [i[1] for i in test_ref_pairs] # 标签
    p = Pool(num_threads)
    all_res = p.map(run_evaluation, zip(test, ref, [evaluator]*len(ref), [metric_kwargs]*len(ref))) # 38张测试图像 预测结果 的指标
    p.close()
    p.join()
    ...
    for i in range(len(all_res)):
        ...
        # append score list for mean 计算平均指标
        for label, score_dict in all_res[i].items():
            if label in ("test", "reference"): # False
                continue
            if label not in all_scores["mean"]: # True
