...
##############################################################################
    ...
    def forward(self, x, subset_idx_list=[14], num = 4):
        ...
        # subset_idx_list=[11]  num = 3保留3个模态 缺失1个模态 val时 num=4 subset_idx_list=[14]
##############################################################################
        # 模拟缺失模态 依据 SUBSETS_MODALITIES 和 subset_idx_list 生成一个布尔型的 numpy 数组 drop，数组中的每个元素表明是否要对 range(4) 里对应的元素进行丢弃操作（True 表示丢弃，False 表示保留）
        drop = np.array([True if k in SUBSETS_MODALITIES[subset_idx_list[0]] else False for k in range(4)]) == False #train 随机[False False True False]  val [False False False False]
##############################################################################
        # print(drop)

        x = torch.chunk(x, 4, dim=1) # 把一个张量按照指定的维度分割成多个较小的张量 (B,4,128,128,128) ->tuple( (B,1,128,128,128),(B,1,128,128,128),(B,1,128,128,128),(B,1,128,128,128) )
        modality_features = []
        final_outputs = []
...
##############################################################################
            ...
            if drop[i]: # 不包括缺失的模态
                ...
                continue
            # ms_outputs (1,32,128,128,128) (1,3,128,128,128) (1,3,64,64,64)  (1,3,32,32,32)  (1,3,16,16,16)  (1,3,8,8,8)
            # feature  (1,320,4,4,4) (1,320,8,8,8) (1,256,16,16,16)  (1,128,32,32,32)  (1,64,64,64,64)  (1,32,128,128,128)
            ms_outputs, feature = self.modality_specific_models[i](x[i])  # 第i个模态图像 对应第i个 unet特征提取网络
            fusion_feature.append(feature) # 全模态4*[(B,320,4,4,4),(B,320,8,8,8),(B,256,16,16,16),(B,128,32,32,32),(B,64,64,64,64),(B,32,128,128,128)]
##############################################################################

            modality_features.append(ms_outputs[0]) #全模态 [(B,32,128,128,128),(B,32,128,128,128),(B,32,128,128,128),(B,32,128,128,128)]
            final_outputs += ms_outputs[1:]  #全模态 4*[(B,3,128,128,128),(B,3,64,64,64),(B,3,32,32,32),(B,3,16,16,16),(B,3,8,8,8)]

##############################################################################


        ms_outputs = self.fusion_decoder(fusion_feature) # ((B,32,128,128,128) (B,3,128,128,128) (B,3,64,64,64)  (B,3,32,32,32)  (B,3,16,16,16)  (B,3,8,8,8))
        modality_features.append(ms_outputs[0]) # [(B,32,128,128,128),(B,32,128,128,128),(B,32,128,128,128),(B,32,128,128,128)] + (B,32,128,128,128)
        final_outputs += ms_outputs[1:] # 4*[(B,3,128,128,128),(B,3,64,64,64),(B,3,32,32,32),(B,3,16,16,16),(B,3,8,8,8)] + [(B,3,128,128,128),(B,3,64,64,64),(B,3,32,32,32),(B,3,16,16,16),(B,3,8,8,8)]
...
##############################################################################
            if drop[i]: # 舍弃的模态 不计算
                ...
                continue
##############################################################################
            attention_maps.append(self.modality_aware_modules[i](torch.cat([modality_features[t], modality_features[-1]], dim=1))) #全模态[(B,32,128,128,128),(B,32,128,128,128),(B,32,128,128,128),(B,32,128,128,128)]
            t = t + 1
...
##############################################################################
        ...
        attention_maps = self.softmax(torch.stack(attention_maps, 1)) #全模态 (B,4,32,128,128,128)


        output = attention_maps[:,0] * modality_features[0] # (B,32,128,128,128)
        for i in range(1, num):
            output += attention_maps[:,i] * modality_features[i] # (B,32,128,128,128)

        output = self.lastconv[0](torch.cat((output, modality_features[-1]), dim = 1)) #  (B,32,128,128,128)
##############################################################################

        output = self.output(output) # (B,3,128,128,128)

        final_outputs = [output, ] + final_outputs # [(B,3,128,128,128) +  5*(B,3,128,128,128),(B,3,64,64,64),(B,3,32,32,32),(B,3,16,16,16),(B,3,8,8,8)]
