...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, subset_index_list: list, subset_size: int, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        assert len(x.shape) == 4, "x must be (c, x, y, z)" # (4, 140, 187, 134)
        ...
        data, slicer = pad_nd_image(x, patch_size, pad_border_mode, pad_kwargs, True, None) # (4, 140, 187, 134) [slice(0, 4, None), slice(0, 140, None), slice(0, 187, None), slice(0, 134, None)]
        data_shape = data.shape  # still c, x, y, z  (4, 140, 187, 134)

        # compute the steps for sliding window
        steps = self._compute_steps_for_sliding_window(patch_size, data_shape[1:], step_size) # [[0, 12], [0, 59], [0, 6]]
        num_tiles = len(steps[0]) * len(steps[1]) * len(steps[2]) # 8
        ...
        # gaussian_filter
        if use_gaussian and num_tiles > 1: # True
            if self._gaussian_3d is None or not all(
                    [i == j for i, j in zip(patch_size, self._patch_size_for_gaussian_3d)]):
                if verbose: print('computing Gaussian')
                gaussian_importance_map = self._get_gaussian(patch_size, sigma_scale=1. / 8) # (128, 128, 128)

                self._gaussian_3d = gaussian_importance_map
                self._patch_size_for_gaussian_3d = patch_size # [128 128 128]
        ...
        if all_in_gpu: # False
            ...
            aggregated_results = np.zeros([self.num_classes] + list(data.shape[1:]), dtype=np.float32) # (3,140,187,134)
            aggregated_nb_of_predictions = np.zeros([self.num_classes] + list(data.shape[1:]), dtype=np.float32) # (3,140,187,134)
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], subset_index_list: list, subset_size: int, mirror_axes: tuple,
        ...
        x = maybe_to_torch(x) # (1, 4, 128, 128, 128)
        result_torch = torch.zeros([1, self.num_classes] + list(x.shape[2:]),
                                   dtype=torch.float) # (1, 3, 128, 128, 128)
        ...
        for m in range(mirror_idx):
            if m == 0:
                ...
                pred = self.inference_apply_nonlin(self(x, subset_index_list, subset_size)[0]) # (B, 3, 128, 128, 128)
