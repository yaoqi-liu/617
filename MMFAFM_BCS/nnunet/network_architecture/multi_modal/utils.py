...
"""根据传入的 subset_size 列表，为每个大小随机选择一个不重复的索引，并将这些索引收集到一个列表中返回"""
def subset_idx(subset_size=[4]):
    ...
    for size in subset_size:
        if size == 1:
            ...
            start, end = 14, 15
        subset_index = np.random.choice(range(start, end), replace=False).tolist() # 从 start 到 end - 1 的范围内随机选择一个索引。replace=False 表示不允许重复选择
